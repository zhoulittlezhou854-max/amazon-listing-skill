...
class LLMClient:
    ...
    def _record_response_meta(
        ...
        *,
        ...
        self._last_response_meta = {
            ...
            "returned_model": response_data.get("model") or self.active_model,
    ...
    def _call_llm(self, system_prompt: str, payload: Dict[str, Any], temperature: float) -> Optional[str]:
        timeout_seconds = self._resolve_request_timeout(payload)
        if self._provider == "openai_compatible":
            text = self._call_openai_compatible(system_prompt, payload, temperature, timeout_seconds=timeout_seconds)
            if text:
                return text
        ...
        if self._provider == "deepseek":
            text = self._call_deepseek(system_prompt, payload, temperature, timeout_seconds=timeout_seconds)
            if text:
                return text
        ...
        try:
            ...
            self._record_response_meta(
                endpoint="chat.completions",
                wire_api="chat/completions",
                response_data={"model": self._model},
                latency_ms=int((time.time() - started) * 1000),
                success=False,
                error=str(exc),
            )
    ...
    def _call_openai_compatible(
        ...
        *,
        ...
        if wire_api == "responses":
            body = {
                "model": self._openai_compatible["model"],
                "input": [
                    {
                        "role": "system",
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                self._record_response_meta(
                    endpoint=endpoint,
                    wire_api=wire_api,
                    response_data={"model": self._openai_compatible["model"]},
                    latency_ms=int((time.time() - started) * 1000),
                    success=False,
                    error=last_error,
                )
        ...
        try:
            ...
            if not isinstance(data, dict):
                self._record_response_meta(
                    endpoint="chat.completions",
                    wire_api="chat/completions",
                    response_data={"model": self._deepseek_model},
                    response_headers=response_headers,
                    latency_ms=int((time.time() - started) * 1000),
                    success=False,
    ...
    def generate_text(self, system_prompt: str, payload: Dict[str, Any], temperature: float = 0.35) -> str:
        """
    ...
    def generate_bullet(self, system_prompt: str, payload: Dict[str, Any], temperature: float = 0.35) -> str:
        return self.generate_text(system_prompt, payload, temperature)

    def generate_description(self, system_prompt: str, payload: Dict[str, Any], temperature: float = 0.4) -> str:
        return self.generate_text(system_prompt, payload, temperature)
