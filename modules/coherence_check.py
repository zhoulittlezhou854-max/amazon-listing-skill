...
def _check_header_body_rupture(bullets: Sequence[str]) -> List[CoherenceIssue]:
    issues: List[CoherenceIssue] = []
    for idx, bullet in enumerate(bullets or [], start=1):
        fluency_issues = check_fluency(f"bullet_b{idx}", str(bullet or ""))
        for issue in fluency_issues:
            if issue.rule_id != "header_body_rupture":
                continue
            issues.append(
                CoherenceIssue(
                    issue_type="header_body_rupture",
                    severity=issue.severity,
                    fields=[f"bullet_b{idx}"],
...
def check_coherence(title: str, bullets: Sequence[str], aplus: str) -> List[CoherenceIssue]:
    ...
    issues.extend(_check_header_body_rupture(bullets))
    return issues
