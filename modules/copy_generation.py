...
def _patch_title_missing_keywords(
    ...
    for keyword in [str(item).strip() for item in (missing_keywords or []) if str(item).strip()]:
        ...
        for keep_count in range(len(sections) - 1, 0, -1):
            ...
            candidate = _ensure_title_core_category_frontload(candidate, payload)
...
def _build_deterministic_title_candidate(
    ...
    lead = " ".join(part for part in [brand, lead_keyword] if part).strip()
    segments: List[str] = [lead] if lead else []
    for keyword in ordered_keywords:
        if keyword and _normalize_keyword_text(keyword) not in _normalize_keyword_text(", ".join(segments)):
            segments.append(keyword)
    for spec in list(_flatten_tokens(numeric_specs))[:2]:
        cleaned = str(spec).strip()
        if cleaned and _normalize_keyword_text(cleaned) not in _normalize_keyword_text(", ".join(segments)):
            segments.append(cleaned)
    scene_phrase = " ".join(
        str(item).replace("_", " ") for item in (payload.get("scene_priority") or [])[:1] if item
    ).strip()
    if scene_phrase:
        segments.append(f"{_title_scene_connector(payload.get('target_language') or 'English')} {scene_phrase}")

    while segments:
        candidate = ", ".join(segment for segment in segments if segment).strip(" ,")
        ...
        if passed and candidate.count(",") >= 2 and len(candidate) <= max_length:
            return candidate
        if len(segments) <= 3:
            break
        segments.pop()
    return _trim_to_word_boundary(", ".join(segment for segment in segments if segment).strip(" ,"), max_length)
...
def _enforce_bullet_length(text: str, limit: int = 300) -> str:
    if len(text) <= limit:
        return text
    if " – " not in text:
        return _trim_to_word_boundary(text, limit).rstrip(" ,;:-|–—") + "."
...
def _compose_exact_match_title(payload: Dict[str, Any]) -> str:
    ...
    max_length = int(payload.get("max_length") or 150)
    ...
    scene_segment = _build_title_scene_segment(scene_payload)

    candidates: List[str] = []
    exact_segment = ", ".join(formatted_secondary_exact)
    spec_segment = ", ".join(specs[:2])
    lead = " ".join(part for part in [brand, lead_exact] if part).strip()

    layouts = [
        [lead, spec_segment, exact_segment, scene_segment],
        [lead, exact_segment, spec_segment, scene_segment],
        [lead, spec_segment, exact_segment],
        [lead, exact_segment, spec_segment],
        [lead, scene_segment, spec_segment],
        [lead, scene_segment],
    ]
    for layout in layouts:
        candidate = ", ".join(part for part in layout if part).strip(" ,")
    ...
    fallback = candidates[0] if candidates else lead
...
def _generate_bullet_points_legacy(
    ...
    for i, bullet_text in enumerate(localized_bullets):
        ...
        if enforced != cleaned and audit_log is not None:
            _log_action(audit_log, f"bullet_b{i+1}", "trimmed", {"reason": "exceeded 300 char limit"})
        cleaned = enforced
...
def _generate_bullets_from_blueprint(
    ...
    for idx, bullet_text in enumerate(localized_bullets):
        ...
        enforced = _enforce_bullet_length(cleaned)
        if enforced != cleaned and audit_log is not None:
            _log_action(audit_log, f"bullet_b{idx+1}", "trimmed", {"reason": "exceeded 300 char limit"})
...
def _llm_generate_bullet(payload: Dict[str, Any]) -> str:
    ...
    system_prompt = (
        ...
        "Keep each bullet roughly 150-200 characters (hard ceiling 250 characters) and self-regulate length without relying on downstream trimming. "
...
def _llm_generate_title(payload: Dict[str, Any]) -> str:
    ...
    system_prompt = (
        ...
        "2. Follow this structure: [Brand] + [core category name], + [3-4 strongest hard specs / selling points], + [core use scene or audience].\n"
        "3. Use commas to separate the major title sections so the title breathes visually.\n"
        "4. Use Title Case for the major words. Never output a crude lowercase keyword dump.\n"
        "5. Weave the l1_keywords and assigned_keywords naturally into readable premium ecommerce language.\n"
        f"6. Keep it under {max_length} characters.\n"
        ...
        "11. Output ONLY the raw title string, no quotes, no explanations.\n"
...
def _generate_and_audit_title(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            attempt_payload = {
                **payload,
                **_build_title_budget(required_keywords, payload.get("max_length", 150)),
            }
        ...
        normalized = _ensure_title_core_category_frontload(normalized, payload)
        if normalized.count(",") < 2:
            _log_action(audit_log, "title", "llm_retry", {"attempt": attempt, "reason": "missing_comma_structure"})
            continue
        ...
        if len(connector_hits) >= 2:
            ...
            continue
        if len(normalized) > payload.get("max_length", 150):
            trimmed_candidate = _trim_to_word_boundary(normalized, payload.get("max_length", 150))
            ...
            trimmed_candidate = _ensure_title_core_category_frontload(trimmed_candidate, payload)
            repassed, repaired_reason, _ = _validate_field_text(trimmed_candidate, required_keywords, numeric_specs)
            if repassed and trimmed_candidate.count(",") >= 2 and len(trimmed_candidate) <= payload.get("max_length", 150):
                _log_action(
                    audit_log,
                    "title",
                    "llm_success",
            ...
            if repaired_candidate:
                ...
                repaired_candidate = _ensure_title_core_category_frontload(repaired_candidate, payload)
                repaired_candidate = _trim_to_word_boundary(repaired_candidate, payload.get("max_length", 150))
                repassed, repaired_reason, _ = _validate_field_text(repaired_candidate, required_keywords, numeric_specs)
                if repassed and repaired_candidate.count(",") >= 2 and len(repaired_candidate) <= payload.get("max_length", 150):
        ...
        if not passed:
            ...
            if missing_keywords:
                ...
                if patched != normalized:
                    ...
                    if repassed and patched.count(",") >= 2:
                        _log_action(
                            audit_log,
                            "title",
                            "llm_success",
    ...
    if last_candidate:
        ...
        repaired_title = _ensure_title_core_category_frontload(repaired_title, payload)
        ...
        if repassed and repaired_title.count(",") >= 2:
            _log_action(
                audit_log,
                "title",
                "llm_success",
    ...
    fallback = _ensure_title_core_category_frontload(fallback, payload)
    max_length = payload.get("max_length", 150)
    ...
    deterministic_candidate = _ensure_title_core_category_frontload(deterministic_candidate, payload)
    ...
    if repassed and deterministic_candidate.count(",") >= 2:
        _log_action(
            audit_log,
            "title",
            "llm_success",
...
def _fallback_text_for_field(field_name: str,
    ...
    if field_name == "title":
        ...
        lead = " ".join([p for p in [brand, lead_keyword] if p]).strip()
        if category and category.lower() not in lead.lower() and len(lead.split()) < 6 and not exact_keywords:
            lead = f"{lead} {category}".strip()
        specs = [item for item in [capability] + list(_flatten_tokens(numeric_specs))[:2] if item]
        anchor_keywords = _dedupe_keyword_sequence(exact_keywords + list(keywords) + list(l1_keywords) + list(assigned_keywords))
        occupied = _normalize_keyword_text(" ".join([lead] + specs))
        anchor_phrase = ", ".join([
            keyword for keyword in anchor_keywords
            if _normalize_keyword_text(keyword) not in occupied
        ][:2])
        scene_phrase = " & ".join(
            [_format_scene_label(str(item), target_language) or str(item).replace("_", " ") for item in (payload.get("scene_priority") or [])[:2] if item]
        ).strip()
        parts = [lead]
        if specs:
            parts.append(", ".join(specs[:3]))
        if anchor_phrase and _normalize_keyword_text(anchor_phrase) not in _normalize_keyword_text(", ".join(parts)):
            parts.append(anchor_phrase)
        if scene_phrase:
            parts.append(f"{_title_scene_connector(target_language)} {scene_phrase}")
        title = _trim_to_word_boundary(", ".join([part for part in parts if part]).strip(), 150)
...
def generate_multilingual_copy(preprocessed_data: PreprocessedData,
    ...
    title = title_text
    if len(title) > 200:
        _log_action(audit_log, "title", "truncate", {"reason": "exceeded 200 char limit"})
        title = title[:197] + "..."
