...
def _check_fluency(generated_copy: Dict[str, Any], writing_policy: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    bullet_dedup = fc.check_bullet_dimension_dedup(bullets)
    if not bullet_dedup.get("pass"):
        issues.append(
            {
                "rule": "fluency_bullet_dimension_dedup",
                "description": (
                    f'Repeated bullet dimension "{bullet_dedup.get("duplicated_dimension")}" '
                    f'across bullets {bullet_dedup.get("affected_bullets")}'
                ),
                "severity": bullet_dedup.get("severity", "medium"),
                "field": "bullets",
            }
        )
    bullet_bytes = fc.check_bullet_total_bytes(bullets)
    if not bullet_bytes.get("pass"):
        issues.append(
            {
                "rule": "fluency_bullet_total_bytes",
                "description": (
                    f'Bullet total bytes {bullet_bytes.get("total_bytes")} exceed SEO limit '
                    f'{bullet_bytes.get("limit")}'
                ),
                "severity": bullet_bytes.get("severity", "soft"),
                "field": "bullets",
            }
        )
