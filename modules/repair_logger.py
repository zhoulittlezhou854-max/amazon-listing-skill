...
def record_repair(
    *,
    ...
    scores_before_after: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    run_root = _run_root_from_artifact_dir(artifact_dir)
    run_root.mkdir(parents=True, exist_ok=True)
    scores_before_after = scores_before_after or {}
    before_score = int(scores_before_after.get("before", _field_fluency_score(field, original)))
    after_score = int(scores_before_after.get("after", _field_fluency_score(field, repaired)))
    record = RepairRecord(
...
def log_repair(
    ...
    artifact_dir = Path("output") / "runs" / str(run_id).strip() / "step6_artifacts"
    return record_repair(
        ...
        scores_before_after=scores_before_after,
    )
