...
def main():
    ...
    print(f"  Title: {baseline_copy.get('title', '')[:70]}")
    print(f"  A10: {baseline_metrics['a10_total']} (title={baseline_metrics['title_front_80']}, tiering={baseline_metrics['keyword_tiering']})")
    print(f"  COSMO: {baseline_metrics['cosmo_total']} (scene={baseline_metrics['scene_coverage']}, binding={baseline_metrics['capability_binding']})")

    # 保存 baseline 的 preprocessed_data（后续变体复用）
    baseline_preprocessed = Path(OUTPUT_DIR) / "preprocessed_data.json"
    baseline_capability = Path(OUTPUT_DIR) / "capability_check_result.json"
    ...
    for variant_id in VARIANTS:
        ...
        strategy = variant_wp.get('keyword_allocation_strategy', 'N/A')
        scene_order = variant_wp.get('scene_priority', [])[:3]
        print(f"  keyword_allocation_strategy: {strategy}")
        print(f"  scene_priority: {scene_order}...")

        # 复制 variant 的 writing_policy.json 到 output 目录
        shutil.copy2(variant_wp_path, OUTPUT_DIR / "writing_policy.json")

        # 清除 copy_generation 模块缓存，强制重新导入
        for mod in list(sys.modules.keys()):
            if 'copy_generation' in mod:
                del sys.modules[mod]
        ...
        if success:
            ...
            print(f"  Title: {variant_copy.get('title', '')[:70]}")
