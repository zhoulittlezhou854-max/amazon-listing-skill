...
def test_generate_text_allows_override_model_for_deepseek(monkeypatch):
    monkeypatch.setenv("DEEPSEEK_API_KEY", "deepseek-key")
    llm_client.configure_llm_runtime(
        {
            "provider": "deepseek",
    ...
    text = client.generate_text("Reply with exactly READY.", {"probe": "override"}, 0.0, override_model="deepseek-reasoner")

    assert text == "READY"
    assert captured["model"] == "deepseek-reasoner"
    assert client.response_metadata["configured_model"] == "deepseek-reasoner"


def test_deepseek_reasoner_response_ignores_reasoning_content(monkeypatch):
    monkeypatch.setenv("DEEPSEEK_API_KEY", "deepseek-key")
    llm_client.configure_llm_runtime(
        {
            "provider": "deepseek",
    ...
    def _fake_post(url, body, headers, timeout=60, verify_ssl=True):
        return {
            ...
            "choices": [
                {
                    "message": {
                        "reasoning_content": "long hidden chain",
    ...
    text = client.generate_text("Reply with exactly READY.", {"probe": "reasoner"}, 0.0, override_model="deepseek-reasoner")
