...
def test_log_repair_writes_to_output_run_directory(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)

    entry = rl.log_repair(
        run_id="H91lite_US_r13",
        field="title",
        rule_id="title_missing_keyword",
        severity="medium",
        original="Mini camera",
        repaired="Mini camera 1080P",
        repair_success=True,
        attempts=1,
        benchmark_used=False,
        scores_before_after={"before": 5, "after": 10},
    )

    log_path = tmp_path / "output" / "runs" / "H91lite_US_r13" / "repair_log.jsonl"
    summary_path = tmp_path / "output" / "runs" / "H91lite_US_r13" / "repair_summary.json"
    assert log_path.exists()
    assert summary_path.exists()
    assert entry["fluency_score_before"] == 5
    assert entry["fluency_score_after"] == 10
