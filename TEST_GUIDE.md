...
2. 已创建 `run_config.json`，其中文件路径均指向 `sample_data/` 目录。
...
## 2. 执行命令
```bash
python3 main.py --steps 0,3,5,6,7,8,9 --config run_config.json --output-dir output
```
...
如需调试 Node 2/3/4，可运行：
```bash
python3 main.py --steps 0,1,2,4 --config run_config.json
```
...
## 5. 常见问题 & 解决
- **文件路径找不到**：确认 `run_config.json` 中的相对路径在当前仓库根目录执行时可访问。
- **编码问题**：所有样例文件均为 UTF-8，无需额外处理。
- **缺少依赖**：本项目仅依赖标准库，如启动报错请检查 Python 版本或虚拟环境。

---

> 按以上步骤即可完成明天的冒烟测试。若需替换真实数据，只需更新 `sample_data/` 内文件并调整 `run_config.json` 指向即可。
