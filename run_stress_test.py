...
def main():
    ...
    best_total = max(results.items(), key=lambda x: x[1]["metrics"].get("total_score", 0))
    best_tiering = max(results.items(), key=lambda x: x[1]["metrics"].get("keyword_tiering", 0))
    best_scene = max(results.items(), key=lambda x: x[1]["metrics"].get("scene_coverage", 0))
