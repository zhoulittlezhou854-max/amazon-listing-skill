...
- 2026-04-21: Streamlit `run_workspace_workflow()` can return `RUN_FAILED` with an empty run directory when `run_generator_workflow()` raises before `execution_summary.json` is written. Persist a failure summary and surface an explicit UI message that the run has already ended and is not continuing in the background.
- 2026-04-21: When Streamlit has been running for days, browser sessions may still hit a stale server process with old imported symbols even after local files are updated. If runtime errors contradict current source signatures, inspect process start times and restart the Streamlit server before assuming the code on disk is wrong.
- 2026-04-21: Streamlit `run_workspace_workflow(..., dual_version=True)` had drifted from `run_pipeline.py --dual-version`: it stopped after writing `version_a`, `version_b`, and `dual_version_report.md`, so no `hybrid/`, `final_readiness_verdict.json`, or `LISTING_READY.md` were produced. Keep the service-layer dual-version path aligned with the CLI wrapper's hybrid/final-verdict flow.
- 2026-04-21: After hybrid generation was wired into the Streamlit service path, remaining UI drift lived in the presentation layer: result cards and history summaries kept reading `version_a` defaults instead of `final_readiness_verdict`. Fixing service generation alone is not enough; the frontend must consume the final verdict as the source of truth.
Pattern-based kill is safer than parsing ps output because the ps|rg pipeline can surface transient helper PIDs.
Backgrounding Streamlit from a one-shot shell in this harness can leave no persistent listener after the command exits; keeping it in a live PTY session is more reliable.
